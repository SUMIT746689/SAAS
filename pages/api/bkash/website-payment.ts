...
const handleTransaction = ({ student_id, user_id,
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const token = await axios.post(process.env.bkash_grant_token_url, { app_key: process.env.bkash_X_App_Key, app_secret: process.env.bkash_app_secret }, {
                headers: {
        ...
        switch (method) {
            case 'POST':
                ...
                await pay_fees.forEach(async (feesP) => {
                    ...
                    const resStd = await handleTransaction(studentRes);
